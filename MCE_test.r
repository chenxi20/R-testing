...
DGP = function(N,T,a0,a1,sexi,w0bar,sew0,K0bar,seK0,delta,b0,b1,seI,c0,c1,c2,seL,d0,d1,d2,seM,e0,e1,e2,se,stata=F){
  ind=1:N      #individual indicator			
  year=1:T	  #time indicator	
  J=8
...
#Generating output
#----------------------------------------------------------------------------
#Yit=as.matrix((Kit^e1)*(Lit^e2)*(Mit^e3)*exp(rnorm(N,se)))
Yit=(Kit^e0)*(Lit^e1)*(Mit^e2)*as.matrix((exp(wit))*exp(rnorm(N,0,se)))


#Transform the matrix to panel data structure
#----------------------------------------------------------------------------
Panel=matrix (NA, nrow=T,ncol=J)
Panel[,1]=t(rep(1,T))
Panel[,2]=t(c(1:T))
Panel[,3]=t(Yit[1,]) 
Panel[,4]=t(Lit[1,]) 
...
Panel[,8]=t(wit[1,])
for (i in 2:N){
Panel_tmp=matrix (NA, nrow=T,ncol=J)
Panel_tmp[,1]=t(rep(i,T))
Panel_tmp[,2]=t(c(1:T))
Panel_tmp[,3]=t(Yit[i,]) 
Panel_tmp[,4]=t(Lit[i,]) 
Panel_tmp[,5]=t(Mit[i,]) 
